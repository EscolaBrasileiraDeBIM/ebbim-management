  ...
  fetch('https://sistema.ebbim.com.br/ebbim-api/Controllers/Email/GetEmailsAA.php?idaa=' + id).then((response) => {
    if (response.ok)
    {
      response.json().then(function(data) {
        ...
        data.forEach(email => {
          htmlEmail = '<div class="mb-3 row"> <div class="divInputEmail"> <div class="form-floating"><input type="text" class="form-control" id="inputEmail' + email.id + '" placeholder="Thor@gmail.com" value="' + email.email +'" aria-describedby="floatingInputHelp" onfocus="showBtnEdit(this.id)" onfocusout="hideBtnEdit(this.id, event)" /> <label for="inputEmail ' + email.id + '">E-mail</label> <button class="btn btn-outline-primary btnEdit btnAdd btnRemove" id="btnRemoveEmail" type="button" onclick="removeEmail(this.id)"><i class="tf-icons bx bx-minus iconEdit"></i></button> <button class="btn btn-outline-primary btnEdit" id="btnEditEmail' + email.id + '" type="button" onclick="editEmail(this.id)" onfocusout="hideBtnEdit(this.id, event)"><i class="tf-icons bx bxs-edit iconEdit"></i></button> </div> </div> <div class="divInputPrincipal py-3"> <div class="form-check"> <input name="default-radio-1" class="form-check-input" type="radio" id="inputPrincipal' + email.id + '" value="1" checked="" onchange="editPessoa(this.id)"> </div> </div> </div>';
          /* <div class="mb-3 row">
